...
#     # Track current seed for regeneration
#     current_seed: list[int | None] = [config['SEED']]

#     def regenerate() -> MazeGenerator:
#         """Generate a new maze (new random seed each time)."""
#         import random
#         current_seed[0] = random.randint(0, 2**31)
#         new_config = dict(config)
#         new_config['SEED'] = current_seed[0]
...
def main() -> int:
    ...
    # If a seed is in config, use it for the first maze and every regeneration.
    # If no seed, pick a random one now for the first maze; each regeneration
    # then picks a fresh random seed (different maze every click).
    ...
    def regenerate() -> MazeGenerator:
        ...
        # else: new_config['SEED'] is already the fixed seed — same maze always
        new_gen = build_generator(new_config)
        try:
            write_output(new_gen, config['OUTPUT_FILE'])
        except OSError:
